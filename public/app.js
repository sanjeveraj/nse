...
// ═══════════════════════════════════════════
async function apiCall(params, timeout=18000) {
  ...
  try {
    ...
    if (!r.ok) {
      console.warn('API error:', r.status, params);
      return null;
    }
    ...
    console.warn('apiCall failed:', e.message, params);
    return null;
  }
}
...
function showSimFunds(){ document.getElementById('m-fund-grid').innerHTML=Array(15).fill(0).map((_,i)=>`<div class="fund-card"><div class="fund-val">—</div><div class="fund-lbl">${['P/E Ratio','Forward P/E','P/B Ratio','EPS (TTM)','ROE','ROA','Revenue','Net Income','Profit Margin','Gross Margin','Debt/Equity','Current Ratio','Div Yield','Beta','Avg Volume'][i]}</div></div>`).join(''); document.getElementById('m-ohlc').textContent='Fundamentals not available for this stock'; }
